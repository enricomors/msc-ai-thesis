...
provide an advantage when minimizing emissions, and mbp19 was more energy efficent during the benchmark phase.

% TODO: restart from here, integrate the analysis of the two datasets to support the experiments results
...
Nonetheless, its energy consumption per unit of work is significantly higher, resulting in greater CO$_2$ emissions overall. In contrast, mbp19, despite longer runtimes, consumed much less energy and emitted substantially less CO$_2$. This trade-off highlights 
...
\begin{table}[h!]
    ...
    \begin{tabular}{|cc|ccccc|}
        ...
        CO2eq (kg) & Cost (k€) & $n^P$ & HW & CO$_2$ (kg) & Time (s) & Cost (k€) \\
        \hline
        0.005 & 300 & 1 & none & none & none & none \\
        0.005 & 400 & 1 & mbp19 & $3.35 \times 10^{-6}$ & $5.16$ & $369.68$ \\
        0.01 & 280 & 87 & leonardo & $4.24 \times 10^{-3}$ & $92.04$ & $274.01$ \\
        0.1 & 270 & 96 & leonardo & $4.74 \times 10^{-3}$ & $100.94$ & $267.40$ \\
        0.0001 & 400 & 1 & leonardo & $7.53 \times 10^{-5}$ & $1.24$ & $368.73$ \\
        0.00001 & 400 & 1 & mbp19 & $7.23 \times 10^{-6}$ & $1.53$ & $368.73$ \\
        \hline
    \end{tabular}
    \caption{Experimental results for the CONTINGENCY algorithm, minimizing runtime with constraints on solution cost and CO2 emissions}
    \label{tab:contingency_results_runtime}
\end{table}

Even by looking at the memory consumption we can see that mbp19 outperforms
...
Now, we presents some analysis of data for the Min-Cut/Max-flow algorithms, and some results of tests performed with HADA. First, we take a look at the emissions for the three algorithms.
We can notice from \ref{fig:max_flow_emissions} that the vast majority of configurations tested emitted less than 0.001 kg of CO$_2$, by looking at the IQR for the emission data (\ref{fig:max_flow_emissions_iqr})
we can see how most configurations emissions are below $8 \times 10^{-6}$ kg of CO$_2$.
...
We now present the results of some tests performed on these algorithms with HADA. As already mentioned, the attribute we use as a parameter for these algorithms is the type of implementation used.
As for ANTICIPATE and CONTINGENCY we test HADA using the minimization of CO$_2$ as objective. In this case we also look at the size of the problem (Nodes, Arcs) to see how this could influence 
the results in terms of emissions and other performance metrics.
...
In \ref{tab:bk_results} \ref{tab:eibfs_results} and \ref{tab:hpf_results} we can see how also for the min-cut/max-flow algorithms, when minimizing the Carbon Emissions, the small scale Hardware 
platforms are preferred, since HADA always selects pc and mbp19. 
