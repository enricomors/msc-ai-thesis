...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{imgs/ant_cont_corr_mat.png}
    \caption{Correlation matrix for ANTICIPATE and CONTINGENCY}
    \label{fig:ant_cont_corr_mat}
\end{figure}

As we can see in \ref{fig:ant_cont_corr_mat}, the average and peak memory usage has the highest correlation with the Carbon Emissions of the algorithms. In particular
...
Then, totEnergy has a correlation of $1$ with CO2e, meaning they are perfectly proportional to each other. This reflects the fact that Carbon Emissions, as measured
...
is just a scaling factor. Lastly, we can also point out that the factor which has the most impact on the solution cost (and thus the quality), is the average memory,
having a negative correlation of $-0.47$.
...
on Runtime, Solution cost, and average Memory consumption (in MB), with the objective of minimizing the CO2e emissions (in kg).

\begin{table}[h!]
    \centering
    \begin{tabular}{ccc|ccc}
        \toprule
        \textbf{Time bound (sec)} & \textbf{Cost (keuro)} & \textbf{memAvg (MB)} & \textbf{nScenarios} & \textbf{Hardware} & \textbf{CO2e(kg)} \\
        \midrule
        $42$ & no & no & 1 & mbp19 & $7.23 \times 10^{-6}$ \\
        \bottomrule
    \end{tabular}
