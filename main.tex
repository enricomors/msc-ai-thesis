...
\begin{enumerate}
    ...
    under different configuration. This implies running the algorithms with different configurations on different Hardware platforms;
...
In the HADA paper \cite{DEFILIPPO2022109199} is described how they gathered a dataset containing records describing:
\begin{itemize}
    \item Algorithm configuration;
    \item Time required to find a solution;
    \item Cost of the solution found;
    \item Average memory used by the algorithm.
\end{itemize}

Our goal is to extend this data with metrics that will allow us to asses the impact of the Algorithm Configuration and the Hardware
Architecture on Energy Consumption, and consequently, on the Carbon Footprint of the algorithm.
...
of the algorithms execution. To do so, we can extend the code for running our algorithms with Codecarbon \cite{courty2024codecarbon}.
...
effect of the Greenhouse Gases (GHG) emitted in the determined timeframe. [Green Algorithms]. Accordin to [insert codecarbon docs ref], the carbon footprint can then 
